...
public class ArticleReadApiTest {
    ...
    void restTest() {
        ArticleReadResponse response = articleReadRestClient.get()
                .uri("/v1/articles/{articleId}", 180912933281984516L)
                .retrieve()
    ...
    void readAllInfiniteScrollTest() {
        List<ArticleReadResponse> responses1 = articleReadRestClient.get()
                .uri("/v1/articles/infinite-scroll?boardId=%s&pageSize=%s&lastArticleId=%s".formatted(1L, 5L, 210330756286074880L))
                .retrieve()
                .body(new ParameterizedTypeReference<List<ArticleReadResponse>>() {
                });

        for (ArticleReadResponse response : responses1) {
            System.out.println("response1.getArticleId() = " + response.getArticleId());
        }
        ...
        for (ArticleReadResponse response : responses2) {
            System.out.println("response2.getArticleId() = " + response.getArticleId());
        }
    }
}
